    ...
    if (errors.length === 0) {
      ...
      try {
        ...
        if (emailError) {
          ...
          if (emailError.message?.includes('CORS') || emailError.message?.includes('preflight')) {
            console.warn('CORS error detected. Please ensure the Edge Function has proper CORS headers.');
          }
        } else {
